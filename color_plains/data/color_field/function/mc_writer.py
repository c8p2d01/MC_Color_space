...
def create_ui_markers(file):
    ...
    for i, pitch in enumerate(var.LAYER_OFFSETS):
        for step in range(0, 360, var.AZIMUTH_STEP):
            positional_tags = f'"color_field_ui","sphere_marker","yawstep_{step}","layer_{i}"'
            interaction = f"{{width:{var.INTERACTION_WIDTH}f,height:{var.INTERACTION_HEIGHT}f, Tags:[{positional_tags}]}}"
            sphere_markers.write(\
                f"execute at @s rotated ~{(step + offset) % 360} {pitch} run summon interaction ^ ^ ^{var.RADIUS} {interaction}\n")
